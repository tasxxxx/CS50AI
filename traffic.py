...
def main():
    ...
    # images, labels = load_data("/Users/tasneembenazir/Documents/DYOC/CS50 AI/traffic/gtsrb")
    ...
    for i in range(0, NUM_CATEGORIES):
        ...
        for root, dir, files in os.walk(current_path):
            for img_file in files:
                img = cv2.imread(os.path.join(current_path, img_file))
                img = np.array(img)
                img = np.divide(img, 255)
                img = cv2.resize(img,(IMG_WIDTH, IMG_HEIGHT))
                all_img.append(img)
                all_cat.append(i)
    result = (all_img, all_cat)
    # print(all_img)
    return result
...
def get_model():
    ...
    tf.keras.layers.Conv2D(16, (4, 4), activation = "relu", input_shape = (IMG_WIDTH, IMG_HEIGHT, 3)),
