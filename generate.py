...
class CrosswordCreator():
        ...
        if arcs == None:
            ...
            for x in self.domains:
                for y in self.crossword.neighbors(x):
                    if (y, x) not in arcs and (x, y) not in arcs:
                        arcs.append((x, y))
        while len(arcs) != 0:
            (x, y) = arcs.pop(0)
            if self.revise(x, y):
    ...
    def select_unassigned_variable(self, assignment):
        ...
        min_no_of_var = 100000000000000000
        result = None
        for current_var in unassigned_variables:
            if len(self.domains[current_var]) < min_no_of_var:
                min_no_of_var = len(self.domains[current_var])
                result = current_var
            if len(self.domains[current_var]) == min_no_of_var:
                if len(self.crossword.neighbors(current_var)) >= len(self.crossword.neighbors(result)):
                    result = current_var
        return result
...
def main():
    ...
    # structure = "/Users/tasneembenazir/Documents/DYOC/CS50 AI/crossword/data/structure3.txt"
    # words = "/Users/tasneembenazir/Documents/DYOC/CS50 AI/crossword/data/words2.txt"
    # output = sys.argv[3] if len(sys.argv) == 4 else None
